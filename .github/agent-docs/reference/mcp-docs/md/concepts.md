...
This section explains the core building blocks of MCP and how they fit together.

[ArchitectureHow MCP connects hosts, clients, servers, and LLMs](./architecture)[TransportsHow messages move between clients and servers](./transports)[ToolsLet the model perform actions through your server](./tools)[ResourcesExpose data and content to the model](./resources)[PromptsReusable prompt templates and workflows](./prompts)[RootsDefine boundaries for what a server can access](./roots)[SamplingAsk the model for completions from within a server](./sampling)

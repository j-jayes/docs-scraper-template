...
Template workflows 
      ...
      * Agent routing  [ Agent routing  ](./) Table of contents 
        * How routing works 
        * Basic usage 
        * Fallback on error 
        * Planning mode 
...
[ ](https://github.com/google/adk-docs/edit/main/docs/agents/routing.md "Edit this page on GitHub") [ ](./index.md "View this page as Markdown")
...
When an agent fails, the router is called again with `errorContext` so it can select a fallback. Failover only applies if the agent fails before yielding any events (see How routing works). The following example checks `errorContext.failedKeys` to avoid re-selecting the failed agent:
...
The following example shows a different `RoutedAgent` configuration. See basic usage for the full runner setup.

JavaScript is disabled on your browser.

   

Skip navigation links

  * [Overview](../../../../../index.html)
  * Package
  * [Use](package-use.html)
  * [Tree](package-tree.html)
  * [Deprecated](../../../../../deprecated-list.html)
  * [Index](../../../../../index-all.html)
  * [Search](../../../../../search.html)



  1. [com.google.adk.web.controller](package-summary.html)



Contents

  1. Description
  2. Related Packages
  3. Classes and Interfaces

Hide sidebar  Show sidebar

# Package com.google.adk.web.controller

* * *

package com.google.adk.web.controller

  * Related Packages

Package

Description

[com.google.adk.web](../package-summary.html)

 

[com.google.adk.web.config](../config/package-summary.html)

 

[com.google.adk.web.dto](../dto/package-summary.html)

 

[com.google.adk.web.service](../service/package-summary.html)

 

[com.google.adk.web.websocket](../websocket/package-summary.html)

 

  * Classes

Class

Description

[AgentController](AgentController.html "class in com.google.adk.web.controller")

Spring Boot REST Controller handling agent-related API endpoints.

[ArtifactController](ArtifactController.html "class in com.google.adk.web.controller")

Controller handling artifact-related API endpoints.

[DebugController](DebugController.html "class in com.google.adk.web.controller")

Controller handling debug and tracing endpoints.

[EvaluationController](EvaluationController.html "class in com.google.adk.web.controller")

Controller handling evaluation-related endpoints (mostly placeholder implementations).

[ExecutionController](ExecutionController.html "class in com.google.adk.web.controller")

Controller handling agent execution endpoints.

[GraphController](GraphController.html "class in com.google.adk.web.controller")

Controller handling graph generation endpoints.

[SessionController](SessionController.html "class in com.google.adk.web.controller")

Controller handling session-related API endpoints.




* * *

Copyright (C) 1980\. All rights reserved.
